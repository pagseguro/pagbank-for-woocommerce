<?php
/**
 * Credit Card Blocks Support for WooCommerce Checkout Blocks.
 *
...
/**
 * Class CreditCardBlocksSupport.
 */
final class CreditCardBlocksSupport extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = 'pagbank_credit_card';

	/**
	 * Gateway instance.
	 *
	 * @var CreditCardPaymentGateway
	 */
	private $gateway;
	...
	public function initialize() {
		$this->settings = get_option( 'woocommerce_pagbank_credit_card_settings', array() );
	...
	public function get_payment_method_script_handles() {
		$asset_path = plugin_dir_path( PAGBANK_WOOCOMMERCE_FILE_PATH ) . 'dist/public/blocks/checkout-credit-card.js';
		...
		wp_register_script(
			...
			plugins_url( 'dist/public/blocks/checkout-credit-card.js', PAGBANK_WOOCOMMERCE_FILE_PATH ),
		...
		wp_enqueue_style( 'pagbank-credit-card-blocks' );

		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'pagbank-credit-card-blocks', 'pagbank-for-woocommerce' );
		}

		return array( 'pagbank-credit-card-blocks' );
	}
	...
	public function get_payment_method_data() {
		...
		$installments_enabled               = 'yes' === $this->get_setting( 'installments_enabled', 'no' );
		$maximum_installments               = (int) $this->get_setting( 'maximum_installments', 12 );
		...
		if ( $installments_enabled && ! $transfer_of_interest_enabled ) {
			...
			$amount_in_cents   = Helpers::format_money_cents( $cart_total );
			$installments_plan = ApiHelpers::get_installments_plan_no_interest( $amount_in_cents, $maximum_installments );
		}
		...
		return array(
			...
			'transfer_of_interest_enabled'       => $transfer_of_interest_enabled,
			'maximum_installments_interest_free' => $maximum_installments_interest_free,
