 ...
 */
class GooglePayPaymentGateway extends WC_Payment_Gateway {
	...
	public function init_form_fields(): void {
		$this->form_fields = array(
			'enabled'         => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Habilitar Google Pay', 'pagbank-for-woocommerce' ),
				'default' => 'no',
			),
			'environment'     => array(
			...
			'title'           => array(
				...
				'default'     => __( 'Google Pay', 'pagbank-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'description'     => array(
				...
				'default'     => __( 'Pague de forma rápida e segura com o Google Pay.', 'pagbank-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'logs_enabled'    => array(
	...
	public function process_payment( $order_id ): array {
		try {
			...
			// Get Google Pay token from POST data.
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$google_pay_token = isset( $_POST['pagbank_google_pay-token'] ) ? sanitize_text_field( wp_unslash( $_POST['pagbank_google_pay-token'] ) ) : null;

			if ( empty( $google_pay_token ) ) {
				throw new Exception( __( 'O token do Google Pay não foi encontrado. Por favor, tente novamente.', 'pagbank-for-woocommerce' ) );
			}
	...
	private function save_order_meta_data( WC_Order $order, array $response, array $request ): void {
		...
		$order->update_meta_data( '_pagbank_payment_method', 'google_pay' );
	...
	public function is_available(): bool {
		...
		// Google Pay is only available on Blocks checkout.
