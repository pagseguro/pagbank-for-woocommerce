<?php
/**
 * Pay with PagBank payment gateway.
 *
...
class PayWithPagBankGateway extends WC_Payment_Gateway {
	...
	public function init_form_fields(): void {
		$this->form_fields = array(
			'enabled'         => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Habilitar Pagar com PagBank', 'pagbank-for-woocommerce' ),
				'default' => 'no',
			),
			'environment'     => array(
			...
			'title'           => array(
				...
				'default'     => __( 'Pagar com PagBank', 'pagbank-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'description'     => array(
				...
				'default'     => __( 'Pague usando sua conta PagBank ou cartão de crédito pelo app.', 'pagbank-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'logs_enabled'    => array(
	...
	public function process_payment( $order_id ): array {
		try {
			...
			// Add order details.
			$this->save_order_meta_data( $order, $response, $data, $is_mobile );
	...
	private function save_order_meta_data( WC_Order $order, array $response, array $request, bool $is_mobile ): void {
		...
		if ( $is_mobile && ! empty( $response['deep_links'][0] ) ) {
			$deeplink = $response['deep_links'][0];
			$order->update_meta_data( '_pagbank_deeplink_url', $deeplink['url'] );
		} elseif ( ! empty( $response['qr_codes'][0] ) ) {
			$qr_code = $response['qr_codes'][0];
			$order->update_meta_data( '_pagbank_qrcode_text', $qr_code['text'] );
			$order->update_meta_data( '_pagbank_qrcode_expiration_date', $qr_code['expiration_date'] );

			// Get QR code image URL.
			if ( ! empty( $qr_code['links'] ) ) {
				foreach ( $qr_code['links'] as $link ) {
					if ( $link['rel'] === 'QRCODE.PNG' ) {
						$order->update_meta_data( '_pagbank_qrcode_image', $link['href'] );
						break;
					}
				}
			}
	...
	public function thankyou_page( int $order_id ): void {
		...
		$deeplink_url    = $order->get_meta( '_pagbank_deeplink_url' );
		$qr_code_image   = $order->get_meta( '_pagbank_qrcode_image' );
		$qr_code_text    = $order->get_meta( '_pagbank_qrcode_text' );
	...
	public function enqueue_styles(): void {
		...
		$is_this_method = $this->id === $payment_method;
